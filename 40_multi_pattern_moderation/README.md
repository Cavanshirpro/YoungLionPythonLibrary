# 40. Multi-keyword moderation scan

## Scenario

Scan text for many phrases in one search structure.

## Project structure

```text
40_multi_pattern_moderation/
├── README.md
└── main.py
```
...
```bash
cd 40_multi_pattern_moderation
python main.py
```
...
Scan text for many phrases in one search structure. The example is intentionally small enough to read in one sitting, but the same API is designed to scale into a larger application module.
