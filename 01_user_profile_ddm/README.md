# 01. User profile with DDM

## Scenario

Model a user profile that is easy to access as attributes while remaining serializable.
...
Model a user profile that is easy to access as attributes while remaining serializable. The example is intentionally small enough to read in one sitting, but the same API is designed to scale into a larger application module.
