# 57. Scheduled cleanup job

## Scenario

Schedule a short in-process cleanup task and inspect its state.

## Project structure

```text
57_scheduler_cleanup_job/
├── README.md
└── main.py
```
...
Schedule a short in-process cleanup task and inspect its state. The example is intentionally small enough to read in one sitting, but the same API is designed to scale into a larger application module.
