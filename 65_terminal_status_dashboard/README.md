# 65. Terminal status dashboard

## Scenario

Render a dependency-free CLI table and progress bar.

## Project structure

```text
65_terminal_status_dashboard/
├── README.md
└── main.py
```
...
```bash
cd 65_terminal_status_dashboard
python main.py
```
...
Render a dependency-free CLI table and progress bar. The example is intentionally small enough to read in one sitting, but the same API is designed to scale into a larger application module.
