# 23. Single-pass score normalization

## Scenario

Combine several bulk mutations into one BatchPlan execution.

## Project structure

```text
23_batchplan_user_scores/
├── README.md
└── main.py
```
...
Combine several bulk mutations into one BatchPlan execution. The example is intentionally small enough to read in one sitting, but the same API is designed to scale into a larger application module.
...
```text
Changed: 32
[{'id': 0, 'score': 5.5, 'active': True}, {'id': 1, 'score': 14.3, 'active': True}, {'id': 2, 'score': 23.1, 'active': True}]
```
