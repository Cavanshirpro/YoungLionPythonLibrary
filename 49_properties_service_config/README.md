# 49. Java-style properties config

## Scenario

Manage a simple properties file without another dependency.

## Project structure

```text
49_properties_service_config/
├── README.md
└── main.py
```
...
```bash
cd 49_properties_service_config
python main.py
```
...
Manage a simple properties file without another dependency. The example is intentionally small enough to read in one sitting, but the same API is designed to scale into a larger application module.
...
```text
{'service.name': 'younglion', 'workers': '4'}
```
