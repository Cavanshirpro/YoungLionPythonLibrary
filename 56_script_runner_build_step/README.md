# 56. ScriptRunner build step

## Scenario

Run a child Python script and capture a structured result.

## Project structure

```text
56_script_runner_build_step/
├── README.md
└── main.py
```
...
```bash
cd 56_script_runner_build_step
python main.py
```
...
Run a child Python script and capture a structured result. The example is intentionally small enough to read in one sitting, but the same API is designed to scale into a larger application module.
...
```text
True build step complete 0.5488
```
