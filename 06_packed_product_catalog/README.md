# 06. Packed product catalog

## Scenario

Store many mostly-read product records using PackedDDM.

## Project structure

```text
06_packed_product_catalog/
├── README.md
└── main.py
```
...
Store many mostly-read product records using PackedDDM. The example is intentionally small enough to read in one sitting, but the same API is designed to scale into a larger application module.
...
```text
Product 3
{'object_bytes': 48, 'storage_bytes': 184, 'recursive_bytes': 468, 'nodes': 8, 'keys': 3}
```
