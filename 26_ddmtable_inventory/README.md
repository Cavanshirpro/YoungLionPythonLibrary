...
Select/query/assign over tabular DDM rows.

## Project structure

```text
26_ddmtable_inventory/
├── README.md
└── main.py
```
...
Select/query/assign over tabular DDM rows. The example is intentionally small enough to read in one sitting, but the same API is designed to scale into a larger application module.
