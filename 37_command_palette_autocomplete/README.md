# 37. Command palette autocomplete

## Scenario

Offer weighted prefix suggestions for a desktop app.

## Project structure

```text
37_command_palette_autocomplete/
├── README.md
└── main.py
```
...
```bash
cd 37_command_palette_autocomplete
python main.py
```
...
Offer weighted prefix suggestions for a desktop app. The example is intentionally small enough to read in one sitting, but the same API is designed to scale into a larger application module.
...
```text
[('Open File', 'open'), ('Open Folder', 'folder')]
```
