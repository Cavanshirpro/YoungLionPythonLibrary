# 61. Rate-limited worker

## Scenario

Limit how fast a local worker performs an action.

## Project structure

```text
61_rate_limited_worker/
├── README.md
└── main.py
```
...
Limit how fast a local worker performs an action. The example is intentionally small enough to read in one sitting, but the same API is designed to scale into a larger application module.
