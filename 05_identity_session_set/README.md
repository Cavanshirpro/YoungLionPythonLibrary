# 05. Identity-based live sessions

## Scenario

Keep mutable session objects in a Python set without value-hash problems.

## Project structure

```text
05_identity_session_set/
├── README.md
└── main.py
```
...
Keep mutable session objects in a Python set without value-hash problems. The example is intentionally small enough to read in one sitting, but the same API is designed to scale into a larger application module.
