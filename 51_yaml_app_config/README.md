# 51. Dependency-free YAML config

## Scenario

Use the practical YAML helper for simple application settings.
...
Use the practical YAML helper for simple application settings. The example is intentionally small enough to read in one sitting, but the same API is designed to scale into a larger application module.
...
```text
{'app': {'name': 'YoungLion', 'debug': True}, 'workers': 4}
```
